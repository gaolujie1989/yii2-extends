...
use lujie\plentyMarkets\helpers\PlentyMarketsRestApiHelper;
...
<?php foreach ($openapi['paths'] as $path => $pathMethods): ?>
    ...
    <?php
        ...
        $params = ArrayHelper::map($method['parameters'] ?? [], 'name', static function (array $parameter) use ($openapi) {
            ...
            if (isset($parameter['type']) && $parameter['type'] === 'integer') {
                $parameter['type'] = 'int';
            }
            return [
        ...
        $apiMethod = PlentyMarketsRestApiHelper::getApiMethodName($path, $httpMethod, $params);
        ...
        if ($requestBody) {
            ...
            $properties = $requestBody['schema']['properties'] ?? [];
        ...
        $batchMethod = false;
        if ($successResponse) {
            $properties = $successResponse['schema']['properties'] ?? null;
            $returnSchemaType = $successResponse['schema']['type'] ?? null;
            $ref = $successResponse['schema']['$ref'] ?? null;
            if ($properties === null && $ref) {
                $refPath = strtr(substr($ref, 2), ['/' => '.']);
                $component = ArrayHelper::getValue($openapi, $refPath);
